...
def get_reservations_by_owner(owner_id):
    try:
        # Obtener los restaurantes del owner
        restaurants = Restaurant.query.filter_by(owner_id=owner_id).all()
        restaurant_ids = [restaurant.id for restaurant in restaurants]

        # Obtener las reservas asociadas a esos restaurantes
...
def get_owners():
    try:
        ...
        if not owners:
            return jsonify({"message": "No se encontraron propietarios.", "owners": []}), 200
        return jsonify({"owners": [owner.serialize() for owner in owners]}), 200
    except Exception as e:
        print(f"Error: {e}")
        return jsonify({"error": "Error interno del servidor"}), 500
...
def create_owner():
    ...
    try:
        new_owner = Owner(
            ...
            email=data['email'],
            telephone=data['telephone'],
            password=generate_password_hash(data['password'])  # Encriptar contraseña
        )
        db.session.add(new_owner)
        db.session.commit()
        return jsonify({"message": "Propietario creado exitosamente", "owner": new_owner.serialize()}), 201
...
def update_owner(owner_id):
    ...
    try:
        ...
        # Validar si el correo o teléfono están en uso por otro propietario
        if Owner.query.filter(
            ((Owner.email == owner.email) | (Owner.telephone == owner.telephone)) & (Owner.id != owner.id)
        ).first():
            return jsonify({"error": "El correo o el teléfono ya están en uso por otro propietario"}), 400
...
def get_owner_restaurants(owner_id):
    ...
    restaurants = Restaurant.query.filter_by(owner_id=owner_id).all()
    if not restaurants:
        return jsonify({"message": "El propietario no tiene restaurantes registrados.", "restaurants": []}), 200
    return jsonify([restaurant.serialize() for restaurant in restaurants]), 200
